 ...
 * 9/11/17
 * 
 * Analyzer.cs has been modified to switch between a string to store a word
 * and a char[] to store a word. Concordance.cs may then determine the 
        ...
        void SplitWords(Sentence sent, int pgindex, int sentindex)
        {
            ...
            foreach (string wd in wordStrings)
            {
                if (wd == string.Empty) { continue; }
                     //(wd.ToString() == string.Empty) { continue; }
